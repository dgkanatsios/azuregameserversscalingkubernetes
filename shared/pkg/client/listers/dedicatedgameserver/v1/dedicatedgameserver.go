...
type DedicatedGameServerLister interface {
	...
	List(selector labels.Selector) (ret []*v1.DedicatedGameServer, err error)
...
// List lists all DedicatedGameServers in the indexer.
func (s *dedicatedGameServerLister) List(selector labels.Selector) (ret []*v1.DedicatedGameServer, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1.DedicatedGameServer))
	})
	return ret, err
}
...
type DedicatedGameServerNamespaceLister interface {
	...
	Get(name string) (*v1.DedicatedGameServer, error)
...
// List lists all DedicatedGameServers in the indexer for a given namespace.
func (s dedicatedGameServerNamespaceLister) List(selector labels.Selector) (ret []*v1.DedicatedGameServer, err error) {
...
// Get retrieves the DedicatedGameServer from the indexer for a given namespace and name.
func (s dedicatedGameServerNamespaceLister) Get(name string) (*v1.DedicatedGameServer, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1.Resource("dedicatedgameserver"), name)
	}
	return obj.(*v1.DedicatedGameServer), nil
}
